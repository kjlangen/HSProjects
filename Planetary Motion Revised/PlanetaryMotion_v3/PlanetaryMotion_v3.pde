...
void setup(){
  ...
  for(int i = 0; i < earths.length; i++){
    earths[i] = new Planet(random(0,870),random(0,670),randomMass,random(6,20));
    if(i == 1){
...
void draw(){
  ...
  for(int i = 0; i < starPositions.length; i++){
    fill(255,255,255);
    ellipse(starPositions[i].x,starPositions[i].y,0.8,0.8);
  }
  
  //Gravity being applied sun to planet
  for (int i = 0; i < earths.length; i++){
    //Calculate displacement
    displacement.set(sun1.position.x - earths[i].position.x,sun1.position.y - earths[i].position.y,0);

    //Calculate magnitude of acceleration
    accelerationMag = accelerationCalc(dist(sun1.position.x,sun1.position.y,earths[i].position.x,earths[i].position.y),sun1.mass);
    
    //Calculate distance            
    distance = (dist(sun1.position.x,sun1.position.y,earths[i].position.x,earths[i].position.y));
        
    //Calculate ratio k
    k = (accelerationMag/distance) * 100;
...
//        println("k: " + k);
        ...
        //Add acceleration
        earths[i].acceleration.x += displacement.x*k;
        earths[i].acceleration.y += displacement.y*k;
      }
    }
  }
